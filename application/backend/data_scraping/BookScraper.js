...
async function saveBook(bookTitle, bookAuthor, bookYear, bookISBN, bookImgURL, bookGenres, bookRating1, bookRating2,
    bookRating3, bookRating4, bookRating5) {
...
function readCSVFile() {
    ...
    .on("data", async function (row) {
        ...
        if(validateTitle(row[1].trim()) == true && row[18].trim().length > 2 && row[5].trim().length >= 1 && row[10].trim() == "Hardcover" || row[10].trim() == "Paperback" || row[10].trim() == "Audiobook") {
            ...
            let unfilteredRatingDistribution = row[18].substring(1, row[18].length - 1);

            if(unfilteredGenres.length >= 1 && row[19].trim().length > 0 && row[4].trim().length > 0 && author.length > 0 && unfilteredRatingDistribution.length > 2 && description.length != 0  && releaseDate.length != 0 && imgURL.length != 0) {
                ...
                let splitRatingDistributions = unfilteredRatingDistribution.split(",");
                let ratingDistribution = [];
                for(i in splitRatingDistributions) {
                    ratingDistribution.push((parseInt(splitRatingDistributions[i].trim().substring(1, splitRatingDistributions[i].trim().length - 1))));
                }
